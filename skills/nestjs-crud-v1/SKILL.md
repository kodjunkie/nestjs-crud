...
- `ParamOption.enum` references the internal `SwaggerEnumType` import from `@nestjs/swagger/dist/types/swagger-enum.type`. v2 inlines this type — if you import the same internal path directly, that import breaks in v2 (one-line swap). Your `enum: [...]` usage continues to work in v2 unchanged.
- v2 rewrites default Swagger text (operation summaries + response descriptions) and adds a `@Crud({ swagger: {...} })` override surface. **If you snapshot-test OpenAPI metadata, expect drift on upgrade.** See the `nestjs-crud-migration` skill §Swagger for fixes.
...
join: {
  profile: {
    allow: ['bio', 'avatarUrl'];
  } // never exposes profile.privateKey etc.
}
```
...
const qb = RequestQueryBuilder.create().search({
  $or: [{ name: { $contL: 'alice' } }, { email: { $contL: 'alice' } }],
});
