// Live Redis round-trip proof for PrismaRedisCacheStrategy.
//
...
// than only "the call did not throw". It touches no database — no Prisma
// schema push, no `yarn test:prisma:*` — only Redis.
//
...
const RUN_PREFIX = `redis-cache-spec:prisma:${process.pid}:${Date.now()}:`;
...
describe.each(clientKinds)('PrismaRedisCacheStrategy live Redis round trip [$name]', ({ name, make, ensureReady, pttl, close }) => {
  ...
  afterEach(async () => {
    ...
    const cleanupStrategy = new PrismaRedisCacheStrategy(inspector);
  ...
  it('wrap on a cold key calls fetchFn once; a second wrap for the same key returns the cached value without calling fetchFn again', async () => {
    const client = make();
    const strategy = new PrismaRedisCacheStrategy(client);
  ...
  it('a value written through one strategy instance is visible to a second strategy instance built on a separately constructed client', async () => {
    ...
    const writer = new PrismaRedisCacheStrategy(writerClient);
    ...
    const reader = new PrismaRedisCacheStrategy(readerClient);
