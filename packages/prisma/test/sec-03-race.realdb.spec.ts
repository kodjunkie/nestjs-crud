...
const dialect = (process.env.PRISMA_PROVIDER === 'mysql' ? 'mysql' : 'postgres') as
  | 'postgres'
  | 'mysql';
const runSuite =
  process.env.PRISMA_PROVIDER === 'postgresql' || process.env.PRISMA_PROVIDER === 'mysql';

(runSuite ? describe : describe.skip)(
  `T-09-02 SEC-03 real-DB race regression [${dialect}]`,
  () => {
