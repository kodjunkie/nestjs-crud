// Live Redis round-trip proof for MikroOrmCacheStrategy.
//
// Every other cache-strategy spec in this repository (cache-strategy.spec.ts)
// drives MockCacheStrategy — an in-process Map. This spec constructs a real
// ioredis client and a real node-redis client against Docker Redis (compose
// service `redis`, 127.0.0.1:6399), and asserts against actual server state
// (the raw value, its TTL, and its disappearance after invalidate) rather
// than only "the call did not throw".
//
// This package's jest config uses ts-jest's ESM preset (@mikro-orm/core v7 is
// pure ESM) — Jest globals are imported explicitly from '@jest/globals' to
// match every other spec in this package, per its own ESM-clean convention.
//
...
const RUN_PREFIX = `redis-cache-spec:mikro-orm:${process.pid}:${Date.now()}:`;
...
describe.each(clientKinds)('MikroOrmCacheStrategy live Redis round trip [$name]', ({ name, make, ensureReady, pttl, close }) => {
  ...
  afterEach(async () => {
    ...
    const cleanupStrategy = new MikroOrmCacheStrategy(inspector);
  ...
  it('wrap on a cold key calls fetchFn once; a second wrap for the same key returns the cached value without calling fetchFn again', async () => {
    const client = make();
    const strategy = new MikroOrmCacheStrategy(client);
    const key = `${prefix}miss-then-hit`;
    const fetchFn = jest.fn<() => Promise<{ value: string }>>().mockResolvedValue({ value: 'fresh' });
  ...
  it('a value written through one strategy instance is visible to a second strategy instance built on a separately constructed client', async () => {
    ...
    const writer = new MikroOrmCacheStrategy(writerClient);
    const key = `${prefix}cross-instance`;
    await writer.wrap(
      key,
      jest.fn<() => Promise<{ value: string }>>().mockResolvedValue({ value: 'cross-instance' }),
      5000,
    );
    await close(writerClient);

    const readerClient = make();
    const reader = new MikroOrmCacheStrategy(readerClient);
    const readerFetch = jest.fn<() => Promise<{ value: string }>>().mockResolvedValue({ value: 'must-not-be-returned' });
  ...
  it('invalidate(prefix) removes every key under that prefix, and a following wrap calls fetchFn again', async () => {
    ...
    let calls = 0;
    const fetchFn = jest.fn<() => Promise<{ value: string }>>().mockImplementation(async () => {
      calls += 1;
