...
- **`qs` declared as a direct dependency:** `CrudRequestInterceptor` imports `qs` at runtime, but the package never declared it — resolution relied on package managers hoisting the copy declared by `@nestjs-crud/request`. Works under default npm/yarn layouts; breaks under strict isolation (pnpm default, Yarn Plug'n'Play). Affects all previously published versions.
