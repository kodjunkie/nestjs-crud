...
import { isFalse, isNil } from '@nestjs-crud/util';
...
export class Validation {
  static getValidationPipe(options: CrudOptions, group?: CrudValidationGroups): ValidationPipe {
    return validator && !isFalse(options.validation)
      ...
      : /* istanbul ignore next -- class-validator absent path: validator is a runtime-required peer-dep (no class-validator-less CI cell exists per D-18) */ undefined;
  }

  static createBulkDto<T = any>(options: MergedCrudOptions): any {
    /* istanbul ignore else -- class-validator/class-transformer absent path: both are runtime-required peer-deps (no peer-dep-less CI cell exists per D-18) */
