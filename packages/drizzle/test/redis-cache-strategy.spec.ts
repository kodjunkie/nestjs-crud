// Live Redis round-trip proof for DrizzleCacheStrategy.
//
...
const RUN_PREFIX = `redis-cache-spec:drizzle:${process.pid}:${Date.now()}:`;
...
describe.each(clientKinds)('DrizzleCacheStrategy live Redis round trip [$name]', ({ name, make, ensureReady, pttl, close }) => {
  ...
  afterEach(async () => {
    ...
    const cleanupStrategy = new DrizzleCacheStrategy({ redisClient: inspector });
  ...
  it('wrap on a cold key calls fetchFn once; a second wrap for the same key returns the cached value without calling fetchFn again', async () => {
    const client = make();
    const strategy = new DrizzleCacheStrategy({ redisClient: client });
  ...
  it('a value written through one strategy instance is visible to a second strategy instance built on a separately constructed client', async () => {
    ...
    const writer = new DrizzleCacheStrategy({ redisClient: writerClient });
    ...
    const reader = new DrizzleCacheStrategy({ redisClient: readerClient });
