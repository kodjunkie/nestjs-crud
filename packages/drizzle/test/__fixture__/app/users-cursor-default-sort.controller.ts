 ...
 * sort (`id` ASC) with `limit: 5`. A cursor-mode request with no `?sort=`
 * must resolve to this default rather than 400ing, proving the route's
 * `@Crud({ query: { sort } })` value now reaches `doGetManyCursor`.
 */
@Crud({
