...
```bash
yarn build          # Build all packages (native tsc -b; composite project references walk dependency order)
yarn clean          # Remove lib/ directories and *.tsbuildinfo files
...
```
HTTP Request
  → CrudRequestInterceptor: parse query/parameters, apply @CrudAuth filter/persist, build SCondition search tree
...
- **Install new dependencies unversioned.** Use `yarn add <pkg>`, not `yarn add <pkg>@X.Y.Z`. Lockfile handles reproducibility; the `package.json` range lets Dependabot/Renovate surface upgrades. Pinning at install time inherits whatever was "latest" that day — the codebase then carries that stale line until a breaking migration forces an upgrade. Exception: security advisory, pre-release, or compat lock. Existing-range edits (e.g., `^5.22.0` → `^7.8.0`) don't apply.
...
- **Run `node scripts/check-peer-ranges.js` before cutting a release.** It fails when an internal `@nestjs-crud/*` dependency or peer range differs from the caret of the `lerna.json` version, when a package's own version differs from `lerna.json`, or when an external peer range admits a major no CI cell installs and root `package.json` `peerRangeGaps` gives no written reason for it. Because `lerna version` rewrites internal `dependencies` but not `peerDependencies`, after `npx lerna version X.Y.Z --no-push --no-git-tag-version --yes` run `node scripts/check-peer-ranges.js --fix`, then a real `yarn install` and `yarn install --immutable`. `tests.yml` runs the check on every run, and `release.yml` runs it before tagging or publishing.
