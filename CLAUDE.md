...
- **Adapter feature parity is NOT a guarantee.** When adding a feature to one adapter, audit the other 3 explicitly — current asymmetries: `@Crud({ query: { cache } })` is honored only by TypeORM (silently no-ops on Drizzle/MikroORM/Prisma; throws `CrudCacheNotConfiguredError` on TypeORM if `DataSource.cache` is missing); `relationLoadStrategy: 'query'` is TypeORM-only and bypasses `JoinOption.allow` column constraints; the optional logger surfaces as a separate ctor arg on TypeORM/Drizzle/MikroORM but as a `serviceConfig.logger` field on Prisma (with silent-no-op default vs `new Logger(...)`). New features that "should be universal" need per-adapter implementations OR an explicit "TypeORM-only" caveat in docs.
